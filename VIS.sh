	...
	then
		...
		then 	
			...
			then
				...
				if find ~/Desktop/VIS/Paises/ -name *.isl -exec sed -i -e 's/asegura\/Escritorio/'"$USER"'\/Desktop/g' {} \; && find ~/Desktop/VIS/Codigo/ -name VIS.py -exec sed -i -e 's/Escritorio/Desktop/g' {} \; && find ~/Desktop/ -name VIS.desktop -exec sed -i -e 's/asegura\/Escritorio/'"$USER"'\/Desktop/g' {} \;
				then
		...
		if bash ~/Descargas/idv_5_7_linux64_installer.sh -q
	   	then
			...
			then
				...
				then
					printf "\nProceso terminado con éxito!!\n  \n---------------------- \n  \nModificando los archivos .isl...\n"
					if find ~/Escritorio/VIS/Paises/ -name *.isl -exec sed -i -e 's/asegura/'"$USER"'/g' {} \; && find ~/Escritorio/ -name VIS.desktop -exec sed -i -e 's/asegura/'"$USER"'/g' {} \;
					then
						printf "\nModificado con éxito!\n"
					else
						printf "\nError: No se pudieron modificar los archivos\n"
...
printf "Creado por Anthony Segura García en colaboración entre la Universidad de Costa Rica y el Instituto Meteorológico Nacional de Costa Rica"

print "e-mail: anthony.seguragarcía@ucr.ac.cr y asegura@imn.ac.cr"
